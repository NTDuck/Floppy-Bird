...
def img_rescale(path: str, dimension: tuple[int]):
    return pygame.transform.scale(pygame.image.load(os.path.join(FOLDER, path)), dimension)

def wav_play(path: str):
    return pygame.mixer.Sound(os.path.join(FOLDER, path))
...
SOUND_JUMP = wav_play('sfx_jump.wav')
SOUND_PASS = wav_play('sfx_pass.wav')
SOUND_DEATH = wav_play('sfx_death.wav')
...
class App():
    ...
    def main(self):
        ...
        self.is_death_sound_played = False
        ...
        while True:
            ...
            for pipe in self.pipes:
                if pipe.collide(self.char):
                    self.isStarted = False
                    self.isOver = True
                    if not self.is_death_sound_played:
                        SOUND_DEATH.play()
                        self.is_death_sound_played = True
